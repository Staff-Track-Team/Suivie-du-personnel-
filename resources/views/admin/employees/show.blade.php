    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
            <div class="flex items-center gap-3 mb-1">
                ...
                <h1 class="text-2xl font-bold text-slate-800">{{ $employee->name }}</h1>
            </div>
            <div class="flex items-center gap-2 ml-10">
                <span class="text-sm text-slate-500">{{ $employee->matricule }}</span>
                <span class="px-2 py-0.5 rounded-full bg-blue-100 text-blue-700 text-xs font-bold">Employé</span>
                <span class="px-2 py-0.5 rounded-full text-xs font-bold {{ $employee->status === 'Actif' ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-700' }}">
                    {{ $employee->status }}
                </span>
            </div>
        </div>
        
        <div class="flex gap-2" x-data="{ deleteEmployeeOpen: false }">
            <a href="{{ route('admin.employees.edit', $employee->id) }}" class="px-4 py-2 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-colors flex items-center gap-2 shadow-lg shadow-blue-500/20">
            ...
            <button type="button" @click="deleteEmployeeOpen = true" class="px-4 py-2 bg-white text-red-600 border border-red-200 rounded-xl hover:bg-red-50 transition-colors flex items-center gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <x-confirm-modal 
                ...
                message="Voulez-vous vraiment supprimer définitivement le profil de {{ $employee->name }} ? Toutes les données associées seront perdues."
                confirmText="Oui, supprimer"
            />

            <form id="form-deleteEmployee" x-ref="formdeleteEmployee" action="{{ route('admin.employees.destroy', $employee->id) }}" method="POST" class="hidden">
                @csrf @method('DELETE')
            </form>
        </div>
    </div>
        ...
        <div class="space-y-6 lg:col-span-2">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                <div class="px-6 py-4 bg-slate-50 border-b border-slate-100 flex items-center gap-3">
                    ...
                    <h2 class="font-bold text-slate-800">Profil Employé</h2>
                </div>
                ...
                <div class="p-8">
                    <div class="flex flex-col md:flex-row gap-8 items-center md:items-start">
                        <div class="shrink-0 text-center">
                            ...
                            <span class="text-xs font-mono text-slate-400">{{ $employee->matricule }}</span>
                        </div>
                        ...
                        <div class="flex-1 w-full grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-6">
                            ...
                            <div>
                                <label class="block text-xs uppercase font-bold text-slate-400 mb-1">Département</label>
                                <div class="font-medium text-slate-800">{{ $employee->employeeInfo->department ?? 'Non défini' }}</div>
                            </div>
                            <div>
                                <label class="block text-xs uppercase font-bold text-slate-400 mb-1">Poste / Fonction</label>
                                <div class="font-medium text-slate-800">{{ $employee->employeeInfo->position ?? 'Non défini' }}</div>
                            </div>
                            <div>
                                <label class="block text-xs uppercase font-bold text-slate-400 mb-1">Date d'embauche</label>
                                <div class="font-medium text-slate-800">{{ ($employee->employeeInfo && $employee->employeeInfo->hired_at) ? \Carbon\Carbon::parse($employee->employeeInfo->hired_at)->format('d/m/Y') : '-' }}</div>
                            </div>
                            <div>
                                <label class="block text-xs uppercase font-bold text-slate-400 mb-1">Téléphone</label>
                                <div class="font-medium text-slate-800">{{ $employee->code_phone }} {{ $employee->phone ?? '-' }}</div>
                            </div>
                             <div>
                                <label class="block text-xs uppercase font-bold text-slate-400 mb-1">Ville</label>
                                <div class="font-medium text-slate-800">{{ $employee->employeeInfo->city ?? '-' }}</div>
                            </div>
                            <div>
                                <label class="block text-xs uppercase font-bold text-slate-400 mb-1">Adresse</label>
                                <div class="font-medium text-slate-800">{{ $employee->employeeInfo->address ?? '-' }}</div>
                            </div>
                            <div>
                                <label class="block text-xs uppercase font-bold text-slate-400 mb-1">Genre</label>
                                <div class="font-medium text-slate-800">{{ $employee->gender ?? 'Non spécifié' }}</div>
                            </div>
                            <div>
                                <label class="block text-xs uppercase font-bold text-slate-400 mb-1">Date de naissance</label>
                                <div class="font-medium text-slate-800">{{ $employee->birthday ? \Carbon\Carbon::parse($employee->birthday)->format('d/m/Y') : '-' }}</div>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="space-y-6">
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden hover:shadow-md transition-all duration-300">
                ...
                <div class="p-4 flex flex-col gap-2">
                    <a href="{{ route('admin.employees.edit', $employee->id) }}" class="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-slate-50 text-slate-600 transition-colors">
                    ...
                    <form action="{{ route('admin.employees.toggle-status', $employee->id) }}" method="POST">
                        @csrf @method('PATCH')
                    ...
                    <div>
                        <div class="flex justify-between text-xs font-bold text-indigo-700 mb-1">
                            ...
                            <span>
                                @php
                                    $completion = 0;
                                    if ($employee->name) $completion += 20;
                                    if ($employee->email) $completion += 20;
                                    if ($employee->phone) $completion += 20;
                                    if ($employee->gender) $completion += 15;
                                    if ($employee->profil) $completion += 25;
                                @endphp
                                {{ $completion }}%
                            </span>
                        </div>
